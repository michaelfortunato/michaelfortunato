...
#work(
  ...
  title: [Project: Adding MeshCNN To PyTorch Geometric],
  dates: dates-helper(start-date: "March 2025", end-date: "Present"),
)
- Given explicit permission from Professor Rana Hanocka to add MeshCNN
  @hanockaMeshCNNNetworkEdge2019
  to the popular PyTorch Geometric framework.
- Pull Request 1 (In Review): #link("https://github.com/pyg-team/pytorch_geometric/pull/10222")
- Communicated with the owner of PyTorch Geometric, Dr. Matthias Fey, to coordinate my integration of MeshCNN for his repository.
- Researching CUDA kernel to compute the edges that, if collapsed, would
  _keep the mesh manifold_.
  This GPU based approach could be an improvement from the original
  implementation of the MeshCNN pooling layer.


#work(
  company: "University of Chicago",
  title: [Independent Work: Characterizing Schur Net's Expressivity
    Via Homomorphism Expressivity],
  dates: "March 2025",
)
- Preprint: #link("https://github.com/michaelfortunato/homomorphism-expressivity/blob/main/main.pdf")
- This work used Homomorphism Expressivity
  @zhangCompleteExpressivenessHierarchy2023, a new hierarchy for graph neural network expressivity (as opposed to WL-tests), to compare Schur Nets
...
- Successfully showed that networks like Autobahn which allow GNN designers
  to base their GNNs off a template graph can distinguish sub-graphs in fewer
...
- In contrast, leveraged the new results in
  ...
  expressivity for Schur Nets more generally
  (without computing the homomorphism expressivity for a particular choice of
  sub-graph).
- Completely novel idea and one of the first complete applications of
  Homomorphism Expressivity that I know to date.
- Homomorphism Expressivity allowed me to show readers a clear way of
  computing the benefits verses trade-offs of using spectral GNNs verses
  bespoke GNNs designed around automorphisms to a specific sub-graph.
...
#work(
  ...
  title: [Lead Undergraduate Research Assistant For Professor William Cong and Tengyuan
    Liang],
